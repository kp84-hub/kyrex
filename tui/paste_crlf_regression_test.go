...
// Mobile / Termux regression: a paste delivered as a bracketed-paste KeyMsg
// (Paste:true) followed by an Enter tapped immediately afterwards — i.e. with
// no timing gap, so _lastKeyTime is the paste's own timestamp — must still
// submit. A previous 50ms burst heuristic swallowed that Enter, making
// "paste then Enter" appear to do nothing on fast mobile taps.
func TestPasteThenFastEnterSubmits(t *testing.T) {
	m := pasteRegressionModel(t, 140, 40)
	// Bracketed paste: the trailing newline (if any) stays inside this single
	// KeyMsg; no separate trailing KeyEnter is generated by the terminal.
	nm, _ := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("fix the parser bug"), Paste: true})
	m = nm.(Model)
	// User taps Enter at the same instant as the paste (fast mobile tap).
	nm, _ = m.Update(tea.KeyMsg{Type: tea.KeyEnter})
	m = nm.(Model)
	if len(m.History) == 0 {
		t.Fatal("submit did not happen — fast Enter after paste was swallowed")
	}
	if !strings.Contains(strings.Join(m.History, "\n"), "fix the parser bug") {
		t.Errorf("submitted content missing pasted text; history=%q", m.History)
	}
}
