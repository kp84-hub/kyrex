...
def test_bot_decision_is_routing_only_and_shared_tools_are_not_permissions(
    ...
    # The live request is necessary for the route decision, but telemetry must
    # stay metadata-only just like the older execution-route logger.
...
def test_jev_routed_mail_delegation_uses_shared_gmail_executor_not_repo():
    store = FakeStore()
    target = {
        "id": "email-bot",
        "owner": "alice",
        "status": "running",
        "rift": "/unused-for-gmail",
    }
    ...
    # This regression is specifically the screenshot failure: routed mail must
    # never be submitted as the generic repo/Rift task.
...
def test_first_delegation_target_survives_engine_thread_boundary_and_task_is_unchanged(
    ...
    # Resolve the engine session while the request-thread ContextVar is live,
    # exactly as stream_chat does. Then CLEAR the ContextVar before calling the
    # host delegation hook, modelling Chat's manually-created engine worker
    # thread where ContextVars are not inherited.
