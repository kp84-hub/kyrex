...
_DELEGATED_EDITOR_TITLE_RE = re.compile(
    ...
    r'calendar(?:\\.\\s*.*)?    """True iff *bot* holds EXACTLY the distinct destructive ``cal:delete`` grant.
...
def _canonical_calendar_delete(text: str) -> str:
    """Return the bounded Calendar Editor command for delegated delete text.

    Directly-supported editor grammar is preserved byte-for-byte. The one
    additional form accepted here is the Chief-of-Staff's descriptive
    delegation sentence with an explicitly quoted title; only that quoted
    title is carried into the canonical editor grammar. Anything else fails
    closed before a durable task is created.
    """
    stripped = str(text or "").strip()
    try:
