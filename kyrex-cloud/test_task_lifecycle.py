...
def test_same_session_serialized_across_workers():
    ...
    # Worker A claims and runs t1.
    assert worker_a.claim_and_execute_once(timeout=5.0) is True
    # While t1 is running under this session, worker B must NOT claim t2
    # (same session key is serialised).
    assert store.status(t1) == ts.STATUS_DONE
    assert store.status(t2) == ts.STATUS_QUEUED, "t2 must stay queued until session frees"

    # Different-Bot tasks may run concurrently (sanity: other session claims).
    ...
    # Now the serial session is free; t2 can run on either worker.
    assert worker_b.claim_and_execute_once(timeout=5.0) is True
